...
// ── SLIP CONTENT (legs or screenshot) ────────────────────────────
...
function BuyModal({ slip, tipsterName, onClose }: {
  ...
  async function pay() {
    ...
    onClose(true)  // triggers card to expand with content immediately
  }
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--muted)' }}>{tipsterName} · {slip.legs?.length ?? slip.leg_count} legs · odds {slip.total_odds.toFixed(2)}</div>
              </div>
...
function SlipCard({ slip, tipsterName, tipsterUsername, onBuy, showContent }: {
  ...
  return (
    <div style={{ background: 'var(--card)', borderRadius: 16, border: `1px solid ${slip.total_odds >= 10 ? 'rgba(245,166,35,0.35)' : 'var(--line)'}`, borderLeft: slip.total_odds >= 10 ? '3px solid var(--gold)' : undefined, marginBottom: 10, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 14px 10px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--gold)', lineHeight: 1 }}>{slip.total_odds.toFixed(2)}</div>
      ...
      {/* Slip content — shown after payment or for finished slips */}
...
export default function SlipsPage() {
  ...
  const allSlips: { slip: Betslip; tipsterName: string; tipsterUsername: string }[] = []
  MOCK_TIPSTERS.forEach(t => {
    (MOCK_BETSLIPS[t.id] ?? []).forEach(s => allSlips.push({ slip: s, tipsterName: t.name, tipsterUsername: t.username }))
  })
  allSlips.sort((a, b) => new Date(b.slip.posted_at).getTime() - new Date(a.slip.posted_at).getTime())

  const oddsFilter = getOddsFilter(filter)
  const oddsQuery  = parseOddsQuery(query)
  const filtered   = allSlips.filter(({ slip, tipsterName }) => {
    if (oddsQuery) return slip.total_odds >= oddsQuery.min && slip.total_odds <= oddsQuery.max + 0.99
    if (filter !== 'All') { if (slip.total_odds < oddsFilter.min || slip.total_odds > oddsFilter.max) return false }
    if (query.trim() && !oddsQuery) { const q = query.toLowerCase(); return tipsterName.toLowerCase().includes(q) || slip.total_odds.toString().includes(q) }
    return true
  })
