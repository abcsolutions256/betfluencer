...
import { MOCK_TIPSTERS, MOCK_BETSLIPS } from '@/lib/mockData'
import Link from 'next/link'

type MineTab = 'following' | 'purchases' | 'phone'
...
export default function MinePage() {
  ...
  const followedTipsters = MOCK_TIPSTERS.filter(t => follows.includes(t.id))

  // Get all pending slips from followed tipsters (for notification dot)
  const newSlipCount = followedTipsters.reduce((acc, t) => {
    return acc + (MOCK_BETSLIPS[t.id] ?? []).filter(s => s.result === 'pending').length
  }, 0)
  ...
  const tabs = [
    { key: 'following', label: 'Following', icon: Users,    badge: newSlipCount },
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 overflow-y-auto pb-6">
        ...
        <div style={{ background: 'var(--bg2)', padding: '12px 16px 14px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {newSlipCount > 0 && (
            <div style={{ background: 'var(--gold-lt)', border: '1px solid rgba(245,166,35,0.3)', borderRadius: 10, padding: '5px 12px', fontSize: 12, fontWeight: 700, color: 'var(--gold)', display: 'flex', alignItems: 'center', gap: 5 }}>
              <Bell size={13} /> {newSlipCount} new slip{newSlipCount > 1 ? 's' : ''}
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', background: 'var(--bg2)', borderBottom: '1px solid var(--line)' }}>
          {tabs.map(({ key, label, icon: Icon, badge }) => (
            <button key={key} onClick={() => setTab(key as MineTab)} style={{ flex: 1, padding: '11px 0', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 13, fontWeight: tab === key ? 700 : 500, color: tab === key ? 'var(--gold)' : 'var(--muted)', borderBottom: `2px solid ${tab === key ? 'var(--gold)' : 'transparent'}` }}>
              <Icon size={15} />
              {label}
              {badge > 0 && <span style={{ background: 'var(--gold)', color: '#1a0a00', fontSize: 9, fontWeight: 800, padding: '1px 5px', borderRadius: 20, minWidth: 16, textAlign: 'center' }}>{badge}</span>}
            </button>
          ))}
        </div>
            ...
            <>
              {follows.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 0' }}>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 20, lineHeight: 1.6 }}>Follow tipsters to see their new slips here and get notified instantly.</div>
                ...
                <>
                  ...
                  {followedTipsters.map(t => {
                    const tipsterSlips = MOCK_BETSLIPS[t.id] ?? []
                    const liveSlips    = tipsterSlips.filter(s => s.result === 'pending')
                    const latestSlip   = tipsterSlips[0]
                    return (
                      <div key={t.id} style={{ background: 'var(--card)', borderRadius: 16, border: '1px solid var(--line)', marginBottom: 10, overflow: 'hidden' }}>
                        ...
                        {/* Live slips summary */}
                        {liveSlips.length > 0 && (
                          <div style={{ borderTop: '1px solid var(--line)', padding: '10px 14px', background: 'rgba(245,166,35,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--gold)', animation: 'slipblink 1.5s infinite' }} />
                              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--gold)' }}>{liveSlips.length} live slip{liveSlips.length > 1 ? 's' : ''}</span>
                              <span style={{ fontSize: 11, color: 'var(--muted)' }}>· from {liveSlips[0].total_odds.toFixed(2)} odds</span>
                            </div>
                        ...
                        {liveSlips.length === 0 && (
                          <div style={{ borderTop: '1px solid var(--line)', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: 12, color: 'var(--muted)' }}>No live slips right now</span>
                            <Link href={`/channel/${t.username}`} style={{ textDecoration: 'none' }}>
                              <button style={{ fontSize: 11, fontWeight: 700, padding: '5px 12px', background: 'var(--bg3)', color: 'var(--offwhite)', border: '1px solid var(--line)', borderRadius: 20, cursor: 'pointer' }}>Channel →</button>
                            </Link>
                          </div>
                        )}
                      </div>
                    ...
                    <div key={s.id} className="card">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--muted)' }}>Purchased {new Date(s.purchased_at ?? s.started_at).toLocaleDateString()}</div>
                        </div>
      ...
      <BottomNav />
      <style>{`@keyframes slipblink { 0%,100%{opacity:1;} 50%{opacity:0.25;} }`}</style>
    </div>
  )
}
