...
**Migrations 0001–0013. Apply 0002,0003,0004,0005,0006,0007,0008,0009,0010,0011,0012,0013 to the LIVE DB** (0005 = auth/paywall overhaul; 0010 = skip-verified sync + `verify_attempts`; 0011 = multi-country `countries`/`tipster_countries` — **applied to the live DB 2026-07-08**; 0012 = `countries.about_content` per-market About copy, seed generated by `scripts/generate-about-seed.js` — **applied to the live DB 2026-07-15** via `supabase db push`, which also recorded 0011 in the CLI migration history (it had been applied manually without recording); the identical fallback copy in `src/lib/aboutContent.ts` remains the failure-path safety net; 0013 = `betslips.settled_at` for the marketplace Wins 24h view, backfilled with `posted_at` for already-settled slips — **applied to the live DB 2026-07-16** via `supabase db push`).

**Tables:** `profiles`(role: user|tipster|admin), `tipsters`, `betslips`(+`verification_status`,`hidden`,`verify_attempts`,`settled_at`,proof cols), `betslip_legs`, **`betslip_secrets`**(code/site/screenshot — service-role only), `slip_purchases`(`buyer_key` for guests), `slip_verifications`(+`normalized`/`summary`/`total_odds`), `transactions`(ioTec ledger), `payments`(legacy), `earnings`, `platform_settings`, `countries`(per-market config), `tipster_countries`(tipster↔market m:n). View: `tipster_rankings`.
